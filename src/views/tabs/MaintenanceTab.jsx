      ...
      <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100">
        ...
        <form onSubmit={handleStartMaintenance} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div>
            ...
            <select required value={newTask.room} onChange={e=>setNewTask({...newTask, room: e.target.value})} className="w-full p-3 sm:p-3 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-amber-500 bg-slate-50 cursor-pointer">
          ...
          <div>
            ...
            <input required type="text" value={newTask.reason} onChange={e=>setNewTask({...newTask, reason: e.target.value})} className="w-full p-3 sm:p-3 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-amber-500 bg-slate-50" placeholder="Ej. Reparación de AC, Pintura..." />
          </div>
          <button type="submit" className="w-full bg-amber-500 text-white py-3.5 sm:py-3 rounded-xl hover:bg-amber-600 font-bold shadow-md shadow-amber-500/20">
          ...
          <table className="w-full text-left border-collapse min-w-[600px]">
            <thead>
              <tr className="bg-slate-50 text-slate-500 text-xs sm:text-sm uppercase tracking-wider">
                <th className="px-4 py-3 font-medium">ID Tarea</th>
                <th className="px-4 py-3 font-medium">Habitación</th>
                <th className="px-4 py-3 font-medium">Motivo</th>
                <th className="px-4 py-3 font-medium">Fecha Inicio</th>
                <th className="px-4 py-3 font-medium">Estado / Acción</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {currentTasks.map((t) => (
                <tr key={t.id} className="hover:bg-slate-50 transition">
                  <td className="px-4 py-4 font-mono text-sm font-bold text-slate-500">#{t.id}</td>
                  <td className="px-4 py-4 font-bold text-slate-800 whitespace-nowrap">Hab. {t.room}</td>
                  <td className="px-4 py-4 text-sm text-slate-600">{t.reason}</td>
                  <td className="px-4 py-4 text-sm text-slate-600 font-medium whitespace-nowrap">{t.startDate}</td>
                  <td className="px-4 py-4 whitespace-nowrap">
                    {t.status === 'En progreso' ? (
                      <button onClick={() => handleFinish(t.id, t.room)} className="bg-emerald-100 text-emerald-700 hover:bg-emerald-200 px-3 py-2 rounded-lg text-xs font-bold transition flex items-center">
                        <CheckCircle2 size={16} className="mr-1.5"/> Marcar como Listo
                      </button>
                    ) : (
                      <span className="px-3 py-1.5 rounded-lg text-xs font-bold bg-slate-100 text-slate-600 flex items-center w-max">Completado</span>
                    )}
                  </td>
                </tr>
              ))}
