    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 sm:mb-6">
        ...
        <div className="bg-white w-full sm:w-auto p-2 rounded-xl shadow-sm border border-slate-200 flex flex-row items-center justify-between sm:justify-start space-x-2">
          ...
          <select value={exportPeriod} onChange={(e) => setExportPeriod(e.target.value)} className="p-3 sm:p-2 flex-1 sm:flex-none bg-slate-50 border border-slate-200 rounded-lg text-base sm:text-sm outline-none">
          ...
          <button onClick={handleExportCSV} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-3 sm:py-2 rounded-lg text-sm font-bold transition flex items-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col h-80 sm:h-96">
          <h3 className="font-bold text-slate-700 mb-6 flex items-center"><Activity className="mr-2 text-blue-500"/> Ingresos del Mes</h3>
          <div className="flex-1 flex items-end space-x-1 sm:space-x-3 pt-8 pb-2">
            {data.map((h, i) => (
              <div key={i} className="flex-1 h-full bg-slate-50 rounded-t-lg relative flex flex-col justify-end group">
                <div className="w-full bg-gradient-to-t from-blue-600 to-blue-400 rounded-t-lg transition-all duration-700" style={{ height: `${h}%` }}></div>
              </div>
            ))}
          </div>
          <div className="flex justify-between text-[10px] sm:text-xs text-slate-400 mt-2 font-medium border-t border-slate-100 pt-3">
        ...
        <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="space-y-6 pt-2">
            <div>
              <div className="flex justify-between text-xs sm:text-sm mb-2"><span className="font-medium">Ocupadas / Reservadas</span><span className="font-bold">{pctOccupied}%</span></div>
              <div className="w-full bg-slate-100 rounded-full h-3"><div className="bg-rose-500 h-3 rounded-full" style={{width: `${pctOccupied}%`}}></div></div>
            </div>
            <div>
              <div className="flex justify-between text-xs sm:text-sm mb-2"><span className="font-medium">Disponibles</span><span className="font-bold">{pctAvailable}%</span></div>
              <div className="w-full bg-slate-100 rounded-full h-3"><div className="bg-emerald-500 h-3 rounded-full" style={{width: `${pctAvailable}%`}}></div></div>
            </div>
            <div>
              <div className="flex justify-between text-xs sm:text-sm mb-2"><span className="font-medium">En Mantenimiento</span><span className="font-bold">{pctMaintenance}%</span></div>
              <div className="w-full bg-slate-100 rounded-full h-3"><div className="bg-amber-500 h-3 rounded-full" style={{width: `${pctMaintenance}%`}}></div></div>
            </div>
          </div>
        </div>
      </div>
